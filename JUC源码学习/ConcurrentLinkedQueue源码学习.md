...
> ConcurrentLinkedQueue是一个基于链表实现的**无锁CAS**高并发队列
> 
> ps:最近在工作中用到了这个队列，组内同学提醒这个集合会不会在并发的时候有异常，重新看了一下相关的实现，有如下结论:
>
> 1.并发写/迭代不会抛异常;
> 
> 2.极限情况下无法保证读一致，可能a线程迭代读取删除后，b线程仍然迭代读取到了，并且这种情况只会出现在ab线程都迭代到了这个删除点(**iterator.next()**获取到了值)，然后a线程执行**iterator.remove()**删除的情况，这种情况是因为读取和删除不是一个原子操作出现的问题，影响不大。
